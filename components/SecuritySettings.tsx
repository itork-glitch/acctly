...
import { Mail, Smartphone, Shield, CheckCircle, ArrowLeft } from 'lucide-react';
...
export const SecuritySettings = ({ email, username }: SecurityProps) => {
  ...
  const handleSkip2FA = () => {
    console.log('2fa skipped');
  };

  const handleEmailAuth = () => {
    console.log('Email based 2FA selected');
  };

  const handleAppAuth = () => {
    console.log('App 2fa selected');
  };
    ...
    return (
      <div className='flex justify-center items-center gap-2 px-6 pt-6 pb-2'>
        <motion.div
          ...
          animate={{
            scale: currentStep === 'email-verification' ? 1.2 : 1,
            opacity: 1,
          }}
          transition={{ duration: 0.3 }}
        />
        <motion.div
          ...
          initial={{ scaleX: 0 }}
          animate={{ scaleX: currentStep === '2fa' ? 1 : 0 }}
          transition={{ duration: 0.5, ease: 'easeInOut' }}
        />
        <motion.div
          className={`w-2 h-2 rounded-full ${
            currentStep === '2fa' ? 'bg-[#209e5a]' : 'bg-gray-600'
          }`}
          initial={{ scale: 0.8, opacity: 0.6 }}
          animate={{
            scale: currentStep === '2fa' ? 1.2 : 0.8,
            opacity: currentStep === '2fa' ? 1 : 0.6,
          }}
          transition={{ duration: 0.3 }}
        />
      </div>
                  ...
                  <motion.div variants={contentVariants} className='space-y-3'>
                    <motion.div
                      ...
                      <Button
                        ...
                        variant='outline'
                        className='w-full h-auto p-4 border-gray-600 bg-gray-700/30 hover:bg-gray-600/40 hover:border-[#209e5a] text-left flex items-start gap-4 group'>
